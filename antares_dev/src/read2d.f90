subroutine read2d(q_loc,nx_global,ny_global,nx_loc,ny_loc,inputbufx,inputbufy,bufx,bufy,flnm,dsetname)
use hdf5
use common_params
implicit none
include 'mpif.h'
...
if(ny_global .ne. ny_loc) then
  if(myid .eq. 0) then
        q_loc(1-bufx:nx_loc+bufx,1-bufy:ny_loc)=q_temp(1-bufx:nx_loc+bufx,1-bufy:ny_loc)
  endif

  if(myid .eq. nprocs-1) then
        q_loc(1-bufx:nx_loc+bufx,1:ny_loc+bufy)=q_temp(1-bufx:nx_loc+bufx,1:ny_loc+bufy)
  endif

  if(myid .ne. 0 .and. myid .ne. nprocs-1) then
        q_loc(1-bufx:nx_loc+bufx,1:ny_loc)=q_temp(1-bufx:nx_loc+bufx,1:ny_loc)
  endif
else
